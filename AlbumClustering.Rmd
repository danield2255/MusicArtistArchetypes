...
coloredTableDisplay = function(df){
  reattachedProcessed = reattachData(df, dfAll) %>%
    ...
    mutate_at(.vars=vars(-cluster), .funs=funs(ifelse(.<=0, color_tile("maroon", "transparent")(.*c(.<=0)), 
                                    color_tile("transparent", "light green")(.*c(.>=0)))))
    #mutate(avgDanceability = cell_spec(
    #avgDanceability, color = "white", bold = T,
    #background = spec_color(1:4, end = 0.9, option = "A", direction = -1)))
  
  kTable = kable(reattachedProcessed, escape = F, align = "c") %>%
...
customGreen = "#71CA97"
dfReattachedView = dfReattached %>%
  group_by(cluster) %>%
  summarise(avgAcousticness = mean(avgAcousticness))
formattable(as.data.frame(dfReattachedView),  list(`cluster` = formatter(
              "span", style = ~ style(color = "grey",font.weight = "bold"), `avgAcousticness`= color_tile(customGreen, customGreen0))) )
```
