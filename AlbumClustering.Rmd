...
```{r}
reattachedData = reattachData(first5Pcs5Clusts, dfObjectiveOnly) %>%
  filter(Gender == "F")
countOfReattached = countInClusters(reattachedData, c("Genre1"))
valProportionsReattached = countOfReattached %>%
  group_by(cluster) %>%
  mutate(proportion = countInClusts/sum(countInClusts))
...
reattachedData = reattachData(first5Pcs5Clusts, dfObjectiveOnly) %>%
  filter(Gender == "F")
countOfReattached = countInClusters(reattachedData, c("Genre1", "Genre2"))
valProportionsReattached = countOfReattached %>%
  group_by(cluster) %>%
  mutate(proportion = countInClusts/sum(countInClusts))
...
```
It seems that in the cluster made up of the greatest proportion of female artists that pop was the most prevalent genre. 
