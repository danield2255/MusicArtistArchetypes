...
Now the first 5 principle components will be clustered on. First only 4 clusters will be used. 
```{r}
...
## Section 3: Tests For Difference Between Clusters

For testing categorical differences we will use chi-squared tests for independence. 
```{r}
#Do this for all/more?
dfReattached = reattachData(first5Pcs5Clusts, dfObjectiveOnly)
print(colnames(dfReattachedSpotify))
chisq.test(dfReattached$cluster, dfReattached$Gender)
chisq.test(dfReattached$cluster, dfReattached$Group.Solo)
chisq.test(dfReattached$cluster, dfReattached$Genre1)
```
...
summary(aov(totalDurationSeconds ~ cluster, data = dfReattachedSpotify))
summary(aov(avgTempo ~ cluster, data = dfReattachedSpotify))
...
## Section 4: Specific Questions About Clusters
We have determined the model that used the first 5 PC's to form 5 clusters was the best, so now we can try to speculate about the groups which were formed. 


Refrence
```{r}
colnames(dfObjectiveOnly)
colnames(dfAll)
```
...
stackedBarByClust(first5Pcs5Clusts, c("Group.Solo"))
stackedBarByClust(first5Pcs5Clusts, c("Genre1"))
stackedBarByClust(first5Pcs5Clusts, c("Genre1", "Genre2"))
...
Males dominated the billboard charts between 1999 and 2019, and each cluster's proportion of men far outweighed that of women. However, the cluster with the greatest proportion of female representation was cluster 4, our group with high acousticness and low engery. These descriptors could apply to various genere labels however, so some further investigation may be done to see which genres that females are playing in this "chill" cluster. The following graphic shows the distribution of primary genres of albums by female artists across the clusters. 
...
reattachedDataF = reattachData(first5Pcs5Clusts, dfObjectiveOnly) %>%
  filter(Gender == "F")
countOfReattached = countInClusters(reattachedDataF, c("Genre1", "Genre2"))
valProportionsReattached = countOfReattached %>%
  group_by(cluster) %>%
  mutate(proportion = countInClusts/sum(countInClusts))
...
Here, cluster 2 will be focused on because it has the greatest proportion of groups, mostly represented by male groups.
```{r}
