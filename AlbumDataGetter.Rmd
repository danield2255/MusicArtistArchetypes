...
accoladesDf = merge(riaaDf , grammyDf, by = c("Artist", "Album"))
```
...
attrsOfAlbs = songAttrsDf %>%
  ...
  summarize(numSongsOnAlb = n(), 
            ...
            moreCommonMajMin = which.max(Mode), 
            presenceUncommonTS = ifelse(any(TimeSignature != 4), 0, 1), 
            totalExplicit = sum(ifelse(Explicit == FALSE, 0, 1)))%>%
  mutate(proportionExplicit =totalExplicit/n())
...
## Aggregating Data by Album
These operations will take data from various dataframes and group it by album.
